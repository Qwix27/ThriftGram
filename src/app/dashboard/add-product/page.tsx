...
import { TAG_CATEGORIES, TagCategory, addProductTags } from '@/lib/tags';
...
export default function AddProductPage() {
  ...
  const [selectedTags, setSelectedTags] = useState<Array<{ tag: string; category: TagCategory }>>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error, data: productData } = await supabase.from('products').insert({
        shop_id: shop.id,
        name: formData.name,
      ...
      // Add tags to product - NEW FEATURE
      if (selectedTags.length > 0 && productData) {
        await addProductTags(productData.id, selectedTags);
      }
